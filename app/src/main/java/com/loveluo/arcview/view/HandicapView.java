...
public class HandicapView extends View implements HandicapViewInterface {
    ...
    private List<HandicapEntity.DataBean.ListBean.BuyListBean> buyList = new ArrayList<>();

    private List<HandicapEntity.DataBean.ListBean.SellListBean> sellList = new ArrayList<>();

    private BigDecimal max = new BigDecimal(10000);
    ...
    public void initData() {
        for (int i = 0; i < 7; i++) {
            buyList.add(new HandicapEntity.DataBean.ListBean.BuyListBean());
            sellList.add(new HandicapEntity.DataBean.ListBean.SellListBean());
        }
    }
     ...
     * @return 数量
     */
    private String getBuyNumber(int index, List<HandicapEntity.DataBean.ListBean.BuyListBean> list) {
        String number;
        if (max.floatValue() == 0) {
            number = "--";
     ...
     * @return 数量
     */
    private String getSellNumber(int index, List<HandicapEntity.DataBean.ListBean.SellListBean> list) {
        String number;
        if (max.floatValue() == 0) {
            number = "--";
        } else if (list.get(index) != null && list.get(index).getCountUnfilledVolume() != null) {
     ...
     * @return price
     */
    private String getBuyPrice(int index, List<HandicapEntity.DataBean.ListBean.BuyListBean> list) {
        String price;
        if (max.floatValue() == 0) {
            price = "--";
     ...
     * @return price
     */
    private String getSellPrice(int index, List<HandicapEntity.DataBean.ListBean.SellListBean> list) {
        String price;
        if (max.floatValue() == 0) {
            price = "--";
        } else if (list.get(index) != null && list.get(index).getAmountPrice() != null) {
...
//                (int) (paddingStart + ((5f - i) / 5) * itemHeight),
    ...
    public void updateData(String json) {
        ...
        List<HandicapEntity.DataBean.ListBean.BuyListBean> buyListBeans = entity.getData().getList().getBuyList();
        List<HandicapEntity.DataBean.ListBean.SellListBean> sellListBeans = entity.getData().getList().getSellList();
        Collections.reverse(sellListBeans); //倒序排列
...
//            allBuy = new BigDecimal("0");
            ...
            for (int i = 0; i < 7; i++) {
                if (i < buyListBeans.size()) {
                    HandicapEntity.DataBean.ListBean.BuyListBean bean = buyListBeans.get(i);
                    bean.setDepth(bean.getCountUnfilledVolume());
//                    lastDecimal = bean.getDepth();
//                    allBuy = allBuy.add(bean.getCountUnfilledVolume());
                    buyList.add(bean);
                } else {
                    buyList.add(null);
                }
...
//            lastDecimal = new BigDecimal("0");
            ...
            for (int i = 0; i < 7; i++) {
                if (i < sellListBeans.size()) {
                    HandicapEntity.DataBean.ListBean.SellListBean bean = sellListBeans.get(i);
                    bean.setDepth(bean.getCountUnfilledVolume());
//                    lastDecimal = bean.getDepth();
                    sellList.add(bean);
//                    allSell = allSell.add(bean.getCountUnfilledVolume());
                } else {
                    sellList.add(null);
                }
            }
